...
# ---------
# ## 19) Federated Learning — Flower Client (from notebook; adapted)
# ---------
from __future__ import annotations
...
try:
    import flwr as fl
except ImportError:  # friendly hint
    raise ImportError("Please install Flower: pip install flwr==1.*")

from . import config as CFG
from .utils import set_seed, pick_device, torch_loader_kwargs, log
from .data_preprocessing import ECGDataset
from .models import create_model

class PTBXLFlowerClient(fl.client.NumPyClient):
    def __init__(self, model, train_ds, val_ds, is_binary: bool, device):
        self.model = model.to(device)
        self.device = device
        self.is_binary = is_binary
        self.crit = nn.BCEWithLogitsLoss() if is_binary else nn.CrossEntropyLoss()
        self.train_loader = DataLoader(train_ds, **torch_loader_kwargs(True,  CFG.BATCH_SIZE, device.type))
        self.val_loader   = DataLoader(val_ds,   **torch_loader_kwargs(False, CFG.BATCH_SIZE, device.type))
        base_lr = CFG.RECURRENT_LR if model.__class__.__name__.lower() in {"rnnsimple","tinyecglstm","annpooled"} else CFG.BASE_LR
        self.opt = optim.Adam(self.model.parameters(), lr=base_lr)
    ...
    def fit(self, parameters, config):
        self.set_parameters(parameters)
        local_epochs = int(config.get("local_epochs", CFG.FL_LOCAL_EPOCHS))
    ...
    def _train_one_epoch(self):
        ...
        for xb, yb in self.train_loader:
            ...
            logits = self.model(xb)
            loss = self.crit(logits.view(-1), yb.float()) if self.is_binary else self.crit(logits, yb)
            loss.backward()
            if CFG.GRAD_CLIP_NORM: nn.utils.clip_grad_norm_(self.model.parameters(), float(CFG.GRAD_CLIP_NORM))
            self.opt.step()
    ...
    def _eval(self):
        ...
        for xb, yb in self.val_loader:
            ...
            preds = (torch.sigmoid(logits.view(-1)) >= 0.5).long() if self.is_binary else logits.argmax(1)
            correct += (preds == yb).sum().item(); total += xb.size(0)
        loss = loss_sum/max(1,total); acc = correct/max(1,total)
        return loss, acc

def make_client(model_name: str, n_classes: int, train_paths, y_train_enc, val_paths, y_val_enc, *, device):
    is_binary = (n_classes == 2)
    model = create_model(model_name, n_classes, binary=is_binary)
    train_ds = ECGDataset(train_paths, y_train_enc)
    val_ds   = ECGDataset(val_paths,   y_val_enc)
    return PTBXLFlowerClient(model, train_ds, val_ds, is_binary, device)
