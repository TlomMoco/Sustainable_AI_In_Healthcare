...
class ECGDataset(Dataset):
    ...
    seq_len : Optional[int]
        Base sequence length before downsampling. Defaults to CFG.SEQ_LEN.
    ds_factor : Optional[int]
        Downsample factor; effective T = seq_len // ds_factor. Defaults to CFG.DOWNSAMPLE_FACTOR.
    target_leads : int
    ...
    def __getitem__(self, i: int):
        ...
        try:
            ...
            # Be robust to occasional read/parse failures
            x = np.zeros((self.T, self.target_leads), dtype="float32")
...
def make_train_val_loaders(
    ...
    *,
    ...
    batch_size : Optional[int]
        Defaults to CFG.BATCH_SIZE (deep pipeline’s BATCH_SIZE).
    val_frac : float
...
def make_predict_loader(
    ...
    *,
    ...
    batch_size : Optional[int]
        Defaults to CFG.BATCH_SIZE (deep pipeline’s default).
