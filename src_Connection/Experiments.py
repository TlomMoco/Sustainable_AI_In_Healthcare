...
• Provides CLI entry points to:
  - Run the centralized deep-learning pipeline (via src_Connection.Centralized)
...
Where this connects
-------------------
• src_Connection.Centralized : end-to-end centralized training + plots
• src_Connection.Server      : Flower server (federated learning)
• src_Connection.Client      : Flower clients (local training + FedProx/freezing)
• src_Connection.data_loader : make_feature_table (engineered+minimal tables)
• src_Connection.models      : create_model factory for CNN/RNN/LSTM/ANN
• src_Connection.utils       : logging, seeding, DataLoader kwargs
• src_Connection.config      : global switches (KFOLDS, CV_EPOCHS, RUN_TORCH_*...)
...
Usage
-----
python -m src_Connection.Experiments centralized
python -m src_Connection.Experiments federated --clients 4
python -m src_Connection.Experiments cv
"""
...
def run_centralized():
    ...
    Equivalent to:
        python -m src_Connection.Centralized
    """
    log("[RUN] Centralized")
