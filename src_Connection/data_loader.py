...
# ---------
# ## 2–5) Paths, CSV load, Target mapping, Minimal table (from notebook)
# ---------
from __future__ import annotations
...
DB_CSV  = Path(CFG.PTBXL_CSV)
SCP_CSV = Path(CFG.SCP_CSV)
ROOT    = DB_CSV.parent
...
def build_scp_mapping(db: pd.DataFrame, scp_raw: pd.DataFrame):
    ...
    def to_diag_label(scp_codes_dict, min_conf=float(CFG.SCP_MIN_CONF)):
        try:
...
def build_minimal_table(db: pd.DataFrame, to_diag_label, max_records=None) -> pd.DataFrame:
    ...
    if df.empty: raise RuntimeError("No rows produced — check file paths/mappings.")
    log(f"Minimal deep table: n={len(df)}")
    return df

# ---------
# ## 5b/5c) Feature Engineering — stats, bandpower, HR/HRV (from notebook)
# ---------
def _sr_from_cfg(): return 100 if CFG.RECORD_FILE_COL == "filename_lr" else 500
...
def make_feature_table(save_csv: bool = True) -> tuple[pd.DataFrame, pd.DataFrame]:
    db, scp_raw = load_metadata()
    diag_only, valid_codes, to_diag_label = build_scp_mapping(db, scp_raw)
    features_df = build_minimal_table(db, to_diag_label, CFG.MAX_RECORDS)
    ...
    for idx, p in features_df["record_path"].astype(str).items():
        try:
            X = load_waveform_np(p, T=T, factor=CFG.DOWNSAMPLE_FACTOR)
    ...
    # cache
    if save_csv and CFG.SAVE_FEATURES_CSV:
        out = Path(CFG.ART_DIR) / CFG.FEATURES_CSV_NAME
        out.parent.mkdir(parents=True, exist_ok=True)
        feat.to_csv(out)
        log(f"Saved features → {out}")
    return feat, features_df

# ---------
# ## 8) Leakage-safe Split (from notebook)
# ---------
