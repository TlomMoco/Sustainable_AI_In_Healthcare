...
Launch order (multi-terminal):
    $ python -m src.Server         # Terminal 1 (server)
...
• Writes round-level GLOBAL metrics, per-class accuracy, and confusion matrices
  ...
  - `src.results_visualization` reads these CSVs to make plots/ANOVA.
• Reads FL hyperparameters and experiment names from `src.config`.
...
from src.utils import ensure_dir, append_csv_locked
...
def _append_perclass_row(server_round: int, cm: np.ndarray) -> None:
    ...
    for c, a in zip(SUPERCLASSES, accs):
        row[f"acc_{c}"] = f"{a:.4f}"
...
class LoggingFedAvg(fl.server.strategy.FedAvg):
    ...
    Inputs from clients
    -------------------
    • Client.evaluate(...) in src.Client emits:
