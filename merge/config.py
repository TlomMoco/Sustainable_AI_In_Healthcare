...
• Model selection (cnn/lstm/rnn/ann) and per-architecture hints
...
# -------------------------------------------------------------------------
# MODEL controls the architecture factory in src.models.
MODEL = {
    "type": "cnn",   # "cnn" | "lstm" | "rnn" | "ann" (extendable by your models factory)
    "lstm_hidden": 128,
    "lstm_layers": 1,
    "bidirectional": True,
}

# Client-side tuning controls (src.tuning.run_client_cv)
...
#   "basic36"  -> mean/std/rms per lead (36D) - original baseline
#   "engineered" -> full engineered features with HR/HRV/bandpower
FEATURES = {"kind": "engineered"}  # or "basic36" for simple baseline
...
NOTEBOOK = {
    ...
    "LABEL_MODE": "5class",          # "5class" or "10class" (subclass)
    "SCP_MIN_CONF": 0.0,             # Minimum SCP code confidence threshold
    "BINARY_TASK": False,
    ...
    "SEQ_LEN": 600,
    "DOWNSAMPLE_FACTOR": 2,
    "BATCH_SIZE": 24,              # Deep pipeline default (distinct from FL client BATCH_SIZE)
    "EPOCHS": 12,
    "VAL_FRAC": 0.20,              # Validation fraction for centralized runs
    ...
    "RUN_TORCH_ANN":  False,
    "DEEP_HYBRID": False,
    "SAVE_BEST_WEIGHTS": True,     # Save best model weights per architecture
    ...
    "SAVE_FEATURES_CSV": SAVE_FEATURES_CSV,
    "FEATURES_CSV_NAME": FEATURES_CSV_NAME,
...
SAVE_BEST_WEIGHTS = NOTEBOOK["SAVE_BEST_WEIGHTS"]
...
# Export labeling mode for data_loader
LABEL_MODE = NOTEBOOK["LABEL_MODE"]
SCP_MIN_CONF = NOTEBOOK["SCP_MIN_CONF"]

# Helper function for tuning paths (used by Client.py)
def tuning_paths(run_name: str, cid: int, freeze_enabled: bool = None):
    """
