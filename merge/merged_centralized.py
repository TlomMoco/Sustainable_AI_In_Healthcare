...
-----------------------------------
• src.config (as CFG)                     : paths & hyperparameters (single source of truth)
• src.utils                               : set_seed, pick_device, log, sanitize_mps_env, torch_loader_kwargs
• src.data_loader                         : make_feature_table, train_test_split
• src.data_preprocessing                  : make_label_encoder, make_train_val_loaders, ECGDataset
• src.models                              : create_model (factory for CNN/RNN/LSTM/ANN)
• src.results_visualization               : TorchAdapter, evaluate_models, plot_confusion, plot_learning_curves
...
# Import visualization helpers (conditional, may not exist yet)
try:
    from src.results_visualization import (
        TorchAdapter,
        evaluate_models,
        plot_confusion,
    ...
    _HAS_VIZ = False
    log("[Centralized] results_visualization not available; skipping plots")
...
def run_deep_models(seed: int | None = None) -> None:
    ...
    log("[Centralized] Building feature tables...")
    _, features_df = make_feature_table(save_csv=True)

    # Patient-safe boolean masks avoid leakage across splits.
    log("[Centralized] Creating patient-safe train/test split...")
    ...
    log("[Centralized] Encoding labels...")
    ...
    log(f"[Centralized] {n_classes} classes: {list(le.classes_)}")
    log(f"[Centralized] Train: {len(train_paths)}, Test: {len(test_paths)}")

    # ---------- DataLoaders ----------
    log("[Centralized] Building DataLoaders...")
    ...
    def train_model_torch(name: str, epochs: int | None = None):
        ...
        log(f"[Centralized] Training {name.upper()} for {epochs} epochs...")
        ...
        for ep in range(1, epochs + 1):
            ...
            # Track best by validation loss; snapshot weights
            if va_loss < best_val:
                best_val = va_loss
        ...
        log(f"[Centralized] {name.upper()} training complete. Best val loss: {best_val:.4f}")
        return model, hist
    ...
    for name, flag in [
        ...
        ("rnn",  C("RUN_TORCH_RNN",  False)),
        ("lstm", C("RUN_TORCH_LSTM", False)),
        ("ann",  C("RUN_TORCH_ANN",  False)),
    ]:
        if not flag:
            log(f"[Train] Skipped {name.upper()} (flag off)")
            continue
        log(f"[Train] === {name.upper()} ===")
        ...
        log(f"Saved history: {hist_csv}")

    # ---------- Evaluate on TEST (unified interface via TorchAdapter) ----------
    if not _HAS_VIZ or not deep_models_raw:
        log("[Centralized] No models trained or visualization unavailable. Skipping evaluation.")
        return

    log("[Centralized] Evaluating models on test set...")
    try:
        adapters = {
        ...
        # Fallback for TorchAdapter signature without device parameter
        adapters = {
    ...
    log(f"Saved evaluation: {results_csv}")
    ...
    if not unified_df.empty:
        ...
        log(f"Saved confusion matrix for {best_name}")

    # Learning curves (loss/acc) per model
    if histories:
        plot_learning_curves(histories, out_dir=str(viz_dir))
        log("Saved learning curves")
    
    log("[Done] Centralized run complete.")
