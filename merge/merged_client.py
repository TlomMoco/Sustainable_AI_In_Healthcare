...
Trains CNN/RNN/LSTM/ANN defined in src.models on local splits.
...
6) Tuning: optional client-side CV for hyperparameter selection.
7) Flower API: get_parameters / set_parameters / fit / evaluate implement NumPyClient.
8) Logging: append per-round metrics to CSV under RESULTS_DIR.
...
• src.utils: set_seed, ensure_dir, pick_device, sanitize_mps_env, torch_loader_kwargs, append_csv_locked
...
class PTBClient(fl.client.NumPyClient):
    ...
    def __init__(self, cid: int):
        ...
        if CLIENTS <= 1:
            ...
            splits_idx = np.cumsum(counts)
            buckets = np.split(patients, splits_idx[:-1])
        ...
        best_path, cv_csv = tuning_paths(EXPERIMENT["run_name"], self.cid)
        ...
        if EXPERIMENT.get("freeze_enabled", False) and len(self.train_df) < int(FREEZE_THRESHOLD):
            ...
            if hasattr(self.model, "head"):
                # Train only the last linear layer in the head
                head = self.model.head
                if isinstance(head, nn.Sequential):
                    for j, module in enumerate(head):
                        req = (j == len(head) - 1)
    ...
    def _log_metrics(self, round_num: int, acc: float, loss: float, wall_s: float):
        ...
        append_csv_locked(path, row, header)
